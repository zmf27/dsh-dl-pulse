...
export const DL_PULSE_GUIDANCE = '本机已安装 dsh-dl-pulse 插件（Web GUI 输入框左侧实时下载进度，纯文字，事件驱动推送）。协议：任何下载任务向 ~/.dsh/downloads/<名称>.progress 写入两个空格分隔的数字 "done total"（字节数），文件内容一变即实时推送到界面；下载完成或取消时删除该文件；写文件用「先写临时文件再 rename」保证原子性。用户提到「下载进度 / 进度条」时即指本插件，请据此协作。';
/**
...
export function scanDir(dir) {
    ...
    for (const entry of entries) {
        if (!entry.isFile() || !entry.name.endsWith('.progress'))
            continue;
        try {
